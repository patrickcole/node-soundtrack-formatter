...
for ( let i = 0; i < lines.length; i++ ) {
  ...
  if ( digitsFollowedByColon && alphaChars ) {
    ...
    digitsFollowedByColon.index < alphaChars.index ? timeBeforeTitle = true : timeBeforeTitle = false;
    ...
    console.log(title);
    console.log(formatter.formatDuration(time_values));
    console.log('-------------');

    /*
    let alphaChars = /[a-zA-Z]/.exec(title);

    if ( alphaChars ){
      let filteredTitle = title.substr(alphaChars.index, title.length);
      console.log(filteredTitle);
    }
    
    // filter and convert time to total seconds:
    let time = lines[i].substring(digitsFollowedByColon.index, lines[i].length);
    let time_splitOnMS = time.split(".");
    let time_values = time_splitOnMS[0];
  ...
  //let line_value = lines[i].digitsFollowedByColon();
  //console.log(line_value);

  // find all digits
  // console.log( lines[i].digitsFollowedByColon(/\d+/g) );
}
